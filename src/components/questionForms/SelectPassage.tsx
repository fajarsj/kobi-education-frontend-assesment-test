...
import { SelectPassageInterface, SelectPassageFormInterface } from '@/interfaces/FormsInterface'

interface SelectPassageProps {
  id: string
}

const SelectPassage = ({ id }: SelectPassageProps) => {
  const { updateForms, findForms } = useAppStore()
  const defaultForm: SelectPassageInterface = findForms(id, QuestionTypesEnum.SELECT_PASSAGE)
  const [formsValues, setFormValues] = useState<SelectPassageFormInterface>(
    defaultForm.forms || {
      questions: []
    }
  )
            ...
            {formsValues.questions!.length > 0 && (
              <div className="flex flex-col gap-4">
                {formsValues.questions!.map((question, parentIndex) => (
                  ...
                  <div className="flex justify-between" key={`${question}-${parentIndex}`}>
                    ...
                    <p
                      ...
                      onClick={() => handleDeleteFormField(parentIndex)}
                    >
